...
from core.utils import setup_logging, calculate_performance_metrics, ensure_directories
...
from core.notifications import DiscordNotifier, DiscordConfig
...
class EnhancedPaperTradingSystem:
    ...
    def __init__(self, config_file: str = "config/enhanced_paper_trading_config.json"):
        ...
        self.logger.info(f"Initialized EnhancedPaperTradingSystem with ${self.capital:,.0f} capital")
    ...
    def _setup_discord_notifications(self) -> Optional[DiscordNotifier]:
        ...
        try:
            ...
            if os.path.exists(discord_config_path):
                ...
                if discord_config_data.get('enabled', False) and discord_config_data.get('webhook_url') != 'YOUR_DISCORD_WEBHOOK_URL_HERE':
                    config = DiscordConfig(
                        webhook_url=discord_config_data['webhook_url'],
                        bot_name=discord_config_data.get('bot_name', 'Trading Bot'),
                        bot_avatar=discord_config_data.get('bot_avatar', 'https://cdn.discordapp.com/emojis/📈.png'),
        ...
        for strategy_name in strategy_names:
            try:
                if strategy_name == 'regime_ensemble':
                    ...
                    strategies[strategy_name] = strategy_factory.create_strategy(strategy_name)
    ...
    def run_daily_trading(self, date: Optional[date_class] = None):
        ...
        try:
            ...
            for symbol in symbols:
                ...
                signals = self._generate_regime_aware_signals(symbol_data, regime_name, regime_params)
    ...
    def _get_market_data(self, symbols: List[str], date: date_class) -> Optional[pd.DataFrame]:
    ...
    def _generate_regime_aware_signals(self, data: pd.DataFrame, regime_name: str, 
        ...
        for strategy_name, strategy in self.strategies.items():
            try:
                if strategy_name == 'regime_ensemble':
                    ...
                    signals[strategy_name] = strategy_signals.iloc[-1] if not strategy_signals.empty else 0.0
                else:
    ...
    def _generate_base_features(self, data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        volume = data['Volume'] if 'Volume' in data.columns else pd.Series(1.0, index=close.index)
    ...
    def _execute_trades(self, symbol: str, signals: Dict[str, float], date: date_class, regime_params):
        ...
        max_position_size = self.config.get('max_position_size', 0.2) * position_multiplier
    ...
    def _log_daily_summary(self, date: date_class, regime_name: str, confidence: float):
        ...
        if len(self.daily_returns) > 1:
            ...
            sharpe_ratio = returns_series.mean() / (returns_series.std() + 1e-6) * np.sqrt(252)
    ...
    def get_performance_report(self) -> Dict:
        ...
        return {
            ...
            'feature_performance': self.feature_reweighter.get_regime_performance_summary('chop')  # Default to chop regime
        }
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config/enhanced_paper_trading_config.json',
    ...
    try:
        ...
        if args.daily or args.cron:
            ...
            if args.cron and system.discord_notifier:
                system.discord_notifier.send_cron_notification("SUCCESS", "Daily trading completed successfully")
