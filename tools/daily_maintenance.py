...
def sh(cmd: list[str], timeout: int | None = None, env: dict | None = None) -> tuple[int,str,str]:
    p = subprocess.Popen(cmd, cwd=ROOT, env=env or os.environ.copy(),
...
def main():
    ...
    if lock.exists():
        ...
        print(msg); notify_ntfy(f"{REPO}: maintenance SKIP", msg); sys.exit(0)
    ...
    if args.quotes == "ibkr":
        ...
        if not port_open(host, port, 1.0):
            ...
            print(msg); notify_ntfy(f"{REPO}: maintenance FAIL", msg); sys.exit(2)
    ...
    if not args.skip_paper:
        ...
        cmd = [sys.executable, "scripts/paper_runner.py",
               "--symbols", args.symbols, "--poll-sec", str(args.poll_sec),
               "--profile", args.paper_profile, "--steps", str(steps)]
        if args.ntfy:
            cmd.append("--ntfy")
        ...
        daily = read_json(ROOT/f"reports/paper_daily_{datetime.now(timezone.utc).strftime('%Y%m%d')}.json")
        summary["paper"] = {"rc": rc, "fallbacks": meta.get("model_fallbacks",0),
    ...
    if not args.skip_canary:
        ...
        cmd = [sys.executable, "scripts/canary_runner.py",
               "--symbols", args.symbols, "--poll-sec", str(args.poll_sec),
               "--profile", args.live_profile, "--quotes", args.quotes,
               "--shadow", "--steps", str(steps)]
        rc,_,err = sh(cmd, timeout=args.minutes*120)
        rollup_safe()
        ok, valog = validate_canary()
        meta = read_json(ROOT/"reports/canary_run.meta.json")
        daily = read_json(ROOT/f"reports/canary_daily_{datetime.now(timezone.utc).strftime('%Y%m%d')}.json")
        summary["canary"] = {"rc": rc, "validator_ok": ok,
    ...
    title = f"{REPO}: daily maintenance OK" if not summary["alerts"] else f"{REPO}: maintenance ALERT"
    body = json.dumps(summary, indent=2)
    if args.ntfy: notify_ntfy(title, body)
    if summary["alerts"]:
        open_issue_if_needed(title, summary)
        print(body); sys.exit(3)
