...
def reconcile_positions(local_positions: Dict[str, float], venue_positions: Dict[str, float]) -> Dict[str, Any]:
...
def main():
    ...
    ap.add_argument("--output", help="Output file for results (default: reports/reconcile_YYYYMMDD_HHMMSS.json)")
    args = ap.parse_args()
    ...
    try:
        ...
        results = {
            ...
            "has_differences": bool(
                order_diffs["missing_local"] or 
                order_diffs["missing_venue"] or 
                order_diffs["status_mismatches"] or
                position_diffs["qty_diffs"]
            )
        }
