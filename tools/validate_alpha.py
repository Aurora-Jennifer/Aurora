...
def main():
    ...
    overall_metrics = results.get("overall_metrics", {})
    print(
        f"IC (Spearman): {overall_metrics.get('mean_ic', 0.0):.4f} ± {overall_metrics.get('std_ic', 0.0):.4f}"
    )
    print(
        f"Hit Rate: {overall_metrics.get('mean_hit_rate', 0.0):.4f} ± {overall_metrics.get('std_hit_rate', 0.0):.4f}"
    )
    ...
    for fold in fold_summaries:
        print(
            f"  Fold {fold['fold']}: IC={fold['ic_spearman']:.4f}, Hit={fold['hit_rate']:.4f}, N={fold['n_predictions']}"
        )
