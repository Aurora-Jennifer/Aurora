...
def main():
    ...
    parser.add_argument("--eval-output", default="reports/alpha_eval.json", help="Evaluation output path")
    ...
    try:
        ...
        logger.info(f"  IC (Spearman): {overall_metrics['mean_ic']:.4f} ± {overall_metrics['std_ic']:.4f}")
        logger.info(f"  Hit Rate: {overall_metrics['mean_hit_rate']:.4f} ± {overall_metrics['std_hit_rate']:.4f}")
        ...
        logger.info(f"  Hit Rate ≥ 0.52: {'✅' if overall_metrics['mean_hit_rate'] >= 0.52 else '❌'}")
