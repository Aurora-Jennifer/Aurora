...
def test_stale_quote_tripwire():
    ...
    stale_ts = (datetime.now(UTC) - timedelta(seconds=2)).isoformat()
    ...
    provider = MockQuoteProvider({"SPY": stale_quote, "TSLA": fresh_quote})
...
def test_wide_spread_tripwire():
    ...
    provider = MockQuoteProvider({"SPY": narrow_spread, "TSLA": wide_spread})
...
def test_market_closed_tripwire():
    ...
    provider = MockQuoteProvider({"SPY": normal_quote, "TSLA": zero_bid, "AAPL": null_ask})
