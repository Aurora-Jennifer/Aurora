...
def calculate_return_with_costs(predictions: np.ndarray, targets: np.ndarray, 
                              slippage_bps: float = 5.0, fee_bps: float = 1.0) -> float:
    """
...
def create_walkforward_folds(df: pd.DataFrame, n_folds: int = 5, 
                           min_train_size: int = 252) -> List[Tuple[pd.DataFrame, pd.DataFrame]]:
    """
...
def evaluate_fold(train_df: pd.DataFrame, test_df: pd.DataFrame, 
                 model_path: str, config: dict) -> Dict:
    """
...
def evaluate_alpha_model(symbols: List[str], model_path: str = "artifacts/models/linear_v1.pkl",
                        n_folds: int = 5, output_path: str = "reports/alpha_eval.json") -> Dict:
    """
    ...
    logger.info(f"Overall Hit Rate: {overall_metrics['mean_hit_rate']:.4f} ± {overall_metrics['std_hit_rate']:.4f}")
