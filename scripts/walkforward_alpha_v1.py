...
def run_alpha_v1_fold(
    pipeline, X: np.ndarray, y: np.ndarray, prices: np.ndarray, fold: Fold
) -> dict:
    """
    ...
    # Simulate trading
    pnl, ntrades, wins, losses, med_hold = simulate_safe(prices_test, signals.astype(np.int8))
    ret = np.diff(pnl)
...
def main():
    ...
    parser.add_argument("--symbols", nargs="+", default=["SPY", "TSLA"], help="Symbols to test")
    parser.add_argument(
        "--model-path", default="artifacts/models/linear_v1.pkl", help="Path to Alpha v1 model"
    )
    parser.add_argument("--train-len", type=int, default=252, help="Training window length")
    parser.add_argument("--test-len", type=int, default=63, help="Test window length")
    parser.add_argument("--stride", type=int, default=21, help="Stride between folds")
    parser.add_argument("--warmup", type=int, default=60, help="Warmup period")
    parser.add_argument(
    ...
    for fold in folds:
        ...
        try:
            ...
            logger.info(
                f"Fold {fold.fold_id + 1}: Sharpe={metrics['sharpe_nw']:.3f}, "
