...
def test_paper_trading_basic():
    ...
    print(
        f"✅ Max Position Size: {system.config['risk_params']['max_weight_per_symbol']}"
    )
...
def test_paper_trading_scenarios():
    ...
    print("   Expected position: 0.250 (clamped to max)")
    ...
    print("   Expected position: 0.000 (reduce to 0)")
    ...
    position = system.positions.get(symbol, 0.0)
    print(f"   Position after sell signal: {position:.3f}")
    print("   Expected position: 0.000 (no change - reduce-only)")
    print(f"   Test: {'✅ PASS' if abs(position - 0.0) < 0.001 else '❌ FAIL'}")
