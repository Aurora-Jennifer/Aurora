...
class FeatureReweighter:
    ...
    def __init__(self, rolling_window: int = 60, min_periods: int = 30, 
                 reweight_frequency: int = 20, decay_factor: float = 0.95):
        """
    ...
    def update_feature_performance(self, features: Dict[str, pd.Series], 
                                 returns: pd.Series, regime: str, date: pd.Timestamp):
        """
        ...
        for feature_name, feature_series in features.items():
            ...
            if perf.last_updated < date:
                ...
                perf.rolling_ic = perf.rolling_ic * self.decay_factor + rolling_ic * (1 - self.decay_factor)
                perf.rolling_sharpe = perf.rolling_sharpe * self.decay_factor + rolling_sharpe * (1 - self.decay_factor)
                perf.rolling_returns = perf.rolling_returns * self.decay_factor + rolling_returns * (1 - self.decay_factor)
            else:
...
class AdaptiveFeatureEngine:
    ...
    def generate_adaptive_features(self, data: pd.DataFrame, regime: str, 
                                 base_features: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        """
    ...
    def _generate_adaptive_features(self, data: pd.DataFrame, regime: str, 
                                  base_features: Dict[str, pd.Series]) -> Dict[str, pd.Series]:
        ...
        if regime == 'trend':
            ...
            features['trend_adaptive_momentum'] = self._calculate_adaptive_momentum(close, regime_summary)
            features['trend_adaptive_strength'] = self._calculate_adaptive_trend_strength(close, regime_summary)
            
        elif regime == 'chop':
            # Chop-adaptive features
            features['chop_adaptive_oscillation'] = self._calculate_adaptive_oscillation(close, regime_summary)
            features['chop_adaptive_reversion'] = self._calculate_adaptive_reversion(close, regime_summary)
            
        elif regime == 'volatile':
            # Volatile-adaptive features
            features['volatile_adaptive_vol'] = self._calculate_adaptive_volatility(close, regime_summary)
            features['volatile_adaptive_risk'] = self._calculate_adaptive_risk(close, regime_summary)
    ...
    def _calculate_adaptive_trend_strength(self, close: pd.Series, regime_summary: Dict) -> pd.Series:
    ...
    def update_performance(self, features: Dict[str, pd.Series], returns: pd.Series, 
                          regime: str, date: pd.Timestamp):
        """Update feature performance metrics."""
