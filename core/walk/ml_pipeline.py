...
class MLPipeline:
    ...
    def _load_feature_config(self):
        ...
        if config_path.exists():
            ...
            self.feature_names = [
                'ret_1d', 'ret_5d', 'ret_20d', 'sma_20_minus_50',
                'vol_10d', 'vol_20d', 'rsi_14', 'volu_z_20d'
            ]
    ...
    def predict(self, Xte):
        ...
        try:
            ...
            logger.info(f"Raw predictions: min={np.min(predictions):.6f}, max={np.max(predictions):.6f}, mean={np.mean(predictions):.6f}")
            ...
            logger.info(f"Signal distribution: {np.bincount(signals.astype(int) + 1)}")  # +1 to handle -1,0,1
