...
class RegimeDetector:
    ...
    def detect_regime(self, data: pd.DataFrame) -> Tuple[str, float, RegimeParams]:
        ...
        if len(data) < self.lookback_period:
            logger.warning(f"Insufficient data for regime detection: {len(data)} < {self.lookback_period}")
    ...
    def get_regime_adapted_features(self, data: pd.DataFrame, base_features: Dict[str, pd.Series], 
                                  regime_params: RegimeParams) -> Dict[str, pd.Series]:
        """
        ...
        for feature_name, feature_series in base_features.items():
            ...
            if 'lookback' in feature_name.lower() or any(period in feature_name for period in ['5', '10', '20', '50', '100', '200']):
    ...
    def _adjust_feature_lookback(self, data: pd.DataFrame, feature: pd.Series, 
                                adjustment_factor: float) -> pd.Series:
...
class RegimeAwareFeatureEngine:
    ...
    def generate_regime_adapted_features(self, data: pd.DataFrame) -> Tuple[Dict[str, pd.Series], RegimeParams]:
        """
        ...
        logger.info(f"Generated {len(adapted_features)} regime-adapted features for {regime_name} regime")
