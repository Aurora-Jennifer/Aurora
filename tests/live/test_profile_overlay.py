...
def test_live_canary_profile_overlay():
    ...
    assert 0 < float(live.get("per_trade_notional_pct", 0)) <= 0.1, "per_trade_notional_pct must be 0-10%"
    assert 0 < float(live.get("notional_daily_cap_pct", 0)) <= 0.5, "notional_daily_cap_pct must be 0-50%"
