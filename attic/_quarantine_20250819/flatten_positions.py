...
def flatten_all_positions(broker: Broker, dry_run: bool = False) -> dict[str, Any]:
    ...
    for symbol, qty in positions.items():
        ...
        if not dry_run and order_result.get("status") == "error":
            print(
                f"ERROR: Failed to flatten {symbol}: {order_result.get('error')}", file=sys.stderr
            )
...
def main():
    ...
    ap.add_argument(
        "--output", help="Output file for results (default: reports/flatten_YYYYMMDD_HHMMSS.json)"
    )
    args = ap.parse_args()
