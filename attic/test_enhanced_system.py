...
from core.utils import ensure_directories, setup_logging
    ...
    trend_data = pd.DataFrame(
        {
            ...
            "High": 100
            + np.cumsum(np.random.normal(0.1, 0.5, len(dates)))
            + np.random.uniform(0, 2, len(dates)),
            "Low": 100
            + np.cumsum(np.random.normal(0.1, 0.5, len(dates)))
            - np.random.uniform(0, 2, len(dates)),
...
def test_regime_aware_ensemble():
    ...
    if len(non_zero_signals) > 0:
        print(
            f"  ✅ Non-zero signals: {len(non_zero_signals)} ({len(non_zero_signals)/len(signals)*100:.1f}%)"
        )
...
def test_enhanced_paper_trading():
    ...
    if "regime_stats" in report:
        ...
        for regime, stats in report["regime_stats"].items():
            print(
                f"     {regime}: {stats['count']} days, avg confidence: {stats['avg_confidence']:.3f}"
            )
...
def test_integration():
    ...
    try:
        ...
        reweighter.update_feature_performance(
            features, returns, regime_name, data.index[-1]
        )

        # 3. Adaptive features
        adaptive_engine = AdaptiveFeatureEngine(reweighter)
        adaptive_features = adaptive_engine.generate_adaptive_features(
            data, regime_name, features
        )
...
def create_config_file():
    ...
    config = {
        ...
        "regime_detection": {"lookback_period": 252, "confidence_threshold": 0.3},
        "feature_reweighting": {
            "rolling_window": 60,
