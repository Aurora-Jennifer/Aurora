...
class EnsembleStrategyParams(StrategyParams):
    ...
    def __post_init__(self):
        ...
        if self.trend_features is None:
            self.trend_features = [
                'SMA_Crossover_20_180', 'EMA_Crossover_20_180', 
                'MACD_Crossover', 'RSI_Momentum', 'Donchian_Breakout_20'
            ]
        
        if self.mean_reversion_features is None:
            self.mean_reversion_features = [
                'RSI_2_Extreme', 'RSI_3_Extreme', 'ZScore_20', 
                'BB_Touch_Upper', 'BB_Touch_Lower'
            ]
        
        if self.volatility_features is None:
            self.volatility_features = [
                'ATR_Pct', 'Realized_Vol_20', 'ADX_Trend', 
                'Vol_High', 'Vol_Low'
            ]
        
        if self.volume_features is None:
            self.volume_features = [
                'OBV_Slope', 'Volume_ZScore', 'MFI_Overbought', 'MFI_Oversold'
            ]
...
class EnsembleStrategy(BaseStrategy):
    ...
    def __init__(self, params: EnsembleStrategyParams):
        ...
        logger.info(f"Initialized EnsembleStrategy with {len(params.trend_features)} trend features")
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        logger.info(f"Generated ensemble signals with mean absolute value: {abs(signals).mean():.3f}")
    ...
    def _generate_combined_signals(self) -> pd.Series:
        ...
        all_features = (self.params.trend_features + 
    ...
    def plot_ensemble_analysis(self, data: pd.DataFrame, save_path: Optional[str] = None) -> None:
