...
class RegimeAwareEnsembleStrategy(BaseStrategy):
    ...
    def __init__(self, params: RegimeAwareEnsembleParams):
        ...
        logger.info(f"Initialized RegimeAwareEnsembleStrategy with {params.combination_method} combination")
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        if len(data) < self.params.min_periods:
            logger.warning(f"Insufficient data for signal generation: {len(data)} < {self.params.min_periods}")
        ...
        logger.info(f"Generated regime-aware signals for {regime_name} regime (confidence: {confidence:.2f})")
        
        return filtered_signals
    
    def _generate_regime_adapted_features(self, data: pd.DataFrame, regime_params: RegimeParams) -> Dict[str, pd.Series]:
    ...
    def _generate_trend_features(self, close: pd.Series, lookback_adjustment: float) -> Dict[str, pd.Series]:
        ...
        # Trend strength
        features['trend_strength'] = self._calculate_trend_strength(close, int(20 * lookback_adjustment))
        
        return features
    
    def _generate_mean_reversion_features(self, close: pd.Series, lookback_adjustment: float) -> Dict[str, pd.Series]:
    ...
    def _generate_breakout_features(self, high: pd.Series, low: pd.Series, close: pd.Series, 
                                  lookback_adjustment: float) -> Dict[str, pd.Series]:
    ...
    def _generate_volatility_features(self, close: pd.Series, volume: pd.Series, 
                                    lookback_adjustment: float) -> Dict[str, pd.Series]:
    ...
    def _generate_regime_specific_features(self, data: pd.DataFrame, regime_name: str) -> Dict[str, pd.Series]:
    ...
    def _generate_trend_following_signals(self, data: pd.DataFrame, features: Dict[str, pd.Series]) -> pd.Series:
        ...
        trend_features = {
            ...
            'trend_consistency': features.get('trend_consistency', pd.Series(0.0, index=data.index))
        }
    ...
    def _generate_mean_reversion_signals(self, data: pd.DataFrame, features: Dict[str, pd.Series]) -> pd.Series:
        ...
        mean_reversion_features = {
            ...
            'mean_reversion_strength': features.get('mean_reversion_strength', pd.Series(0.0, index=data.index))
        }
    ...
    def _generate_breakout_signals(self, data: pd.DataFrame, features: Dict[str, pd.Series]) -> pd.Series:
        ...
        breakout_features = {
            'donchian_position': features.get('donchian_position', pd.Series(0.5, index=data.index)),
            'breakout_strength': features.get('breakout_strength', pd.Series(0.0, index=data.index)),
    ...
    def _blend_signals_by_regime(self, trend_signals: pd.Series, mean_reversion_signals: pd.Series,
                                breakout_signals: pd.Series, regime_params: RegimeParams) -> pd.Series:
    ...
    def _apply_confidence_filter(self, signals: pd.Series, regime_params: RegimeParams) -> pd.Series:
    ...
    def _update_performance_tracking(self, data: pd.DataFrame, signals: pd.Series):
        ...
        self.rolling_performance = {
            ...
            'rolling_std': signal_returns.rolling(self.params.rolling_window).std().iloc[-1] if not signal_returns.empty else 0.0,
            'rolling_sharpe': rolling_perf.iloc[-1] / (signal_returns.rolling(self.params.rolling_window).std().iloc[-1] + 1e-6) if not signal_returns.empty else 0.0
        }
    ...
    def _calculate_bollinger_bands(self, close: pd.Series, period: int = 20) -> Tuple[pd.Series, pd.Series]:
