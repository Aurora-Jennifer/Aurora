...
class ExampleVenueBroker(Broker):
    ...
    def get_positions(self) -> Dict[str, float]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/accounts/{self.account_id}/positions", timeout=10)
            response.raise_for_status()
            data = response.json()
    ...
    def get_fills(self, since: datetime | None = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = self.session.get(f"{self.base_url}/accounts/{self.account_id}/fills", params=params, timeout=10)
            response.raise_for_status()
            data = response.json()
