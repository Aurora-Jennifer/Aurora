...
class IBKRBroker:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.ib.isConnected():
                self.connected = True
                logger.info(f"Connected to IBKR {'Paper Trading' if self.config.paper_trading else 'Live Trading'}")
    ...
    def _handle_error(self, reqId: int, errorCode: int, errorString: str, advancedOrderRejectJson: str = ""):
        """Handle IBKR errors."""
    ...
    def place_order(self, symbol: str, quantity: int, order_type: str = 'MKT', 
        ...
        try:
            ...
            if order_type == 'MKT':
                ...
                order = StopLimitOrder('BUY' if quantity > 0 else 'SELL', abs(quantity), limit_price, stop_price)
            else:
    ...
    def get_market_data(self, symbol: str, duration: str = '1 D', bar_size: str = '1 min') -> Optional[pd.DataFrame]:
        """
...
def test_ibkr_connection():
    ...
    try:
        ...
        if broker.is_connected():
            print(f"✅ Connected to IBKR {'Paper Trading' if config.paper_trading else 'Live Trading'}")
