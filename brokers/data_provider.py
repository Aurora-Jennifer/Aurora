...
class IBKRDataProvider:
    ...
    def __init__(self, config: IBKRConfig = None, cache_dir: str = "data/ibkr", 
                 use_cache: bool = True, fallback_to_yfinance: bool = True):
        """
    ...
    def _load_from_cache(self, symbol: str, duration: str, bar_size: str) -> Optional[pd.DataFrame]:
    ...
    def _fallback_to_yfinance(self, symbol: str, start_date: datetime, end_date: datetime) -> Optional[pd.DataFrame]:
    ...
    def get_multiple_symbols_data(self, symbols: List[str], duration: str = '1 Y', 
                                bar_size: str = '1 day') -> Dict[str, pd.DataFrame]:
        """
    ...
    def get_daily_data(self, symbol: str, start_date: date_class, 
                      end_date: date_class) -> Optional[pd.DataFrame]:
        """
...
def test_data_provider():
    ...
    try:
        ...
        for symbol in symbols:
            ...
            if data is not None:
                print(f"✅ {symbol}: {len(data)} rows, date range: {data.index[0]} to {data.index[-1]}")
