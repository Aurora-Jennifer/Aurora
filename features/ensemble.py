...
class SignalCombiner:
    ...
    def compute_weights(self, method: str = "rolling_ic", window: int = 60, 
                       min_obs: int = 30, regularization: float = 1e-6) -> Dict[str, float]:
        """
    ...
    def _compute_ridge_weights(self, window: int, min_obs: int, 
                              regularization: float) -> Dict[str, float]:
        ...
        if len(valid_data) < min_obs:
            logger.warning(f"Insufficient data for ridge regression ({len(valid_data)} < {min_obs})")
            return {name: 1.0 / len(self.features) for name in self.features.keys()}
    ...
    def combine(self, normalize: bool = True, cap: float = 1.0) -> Tuple[pd.Series, pd.Series]:
        ...
        logger.info(f"Combined {len(self.features)} features into signal with mean confidence {confidence.mean():.3f}")
    ...
    def _calculate_confidence(self, combined_signal: pd.Series) -> pd.Series:
        ...
        if len(self.features) > 1:
            ...
            feature_signs = pd.DataFrame({name: np.sign(signal) for name, signal in self.features.items()})
...
class RegimeSwitcher:
    ...
    def add_regime_indicator(self, name: str, indicator: pd.Series, 
                           threshold: float, regime_type: str) -> None:
        """
...
if __name__ == "__main__":
    ...
    donchian_signal = ((price_data > high_20.shift(1)) * 1 + 
                       (price_data < low_20.shift(1)) * -1).fillna(0)
